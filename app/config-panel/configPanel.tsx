...
export function ConfigPanel() {
    ...
    const parametersBody = (row: any) => (
        <Button size="small" label={t("config_panel.edit_parameters")}
            onClick={() => {
                ...
                const rows: ParamRow[] = entries.map(([k, v]) => {
                    const display = typeof v === "string" ? v : JSON.stringify(v);
                    return { key: k, value: display, _id: k + "__" + Math.random().toString(36).slice(2) };
                });
                setInputValue(rows);
    ...
    type ParamRow = { key: string; value: string; _id: string };
    const [inputValue, debouncedValue, setInputValue]  = useDebounce<ParamRow[]>([], 500);
    ...
    const addParamRow = () => {
        setInputValue([...inputValue, { key: "", value: "", _id: "row_" + Math.random().toString(36).slice(2) }]);
    };

    const updateParamKey = (id: string, newKey: string) => {
        setInputValue(inputValue.map((r) => r._id === id ? { ...r, key: newKey } : r));
    };

    const updateParamValue = (id: string, newValue: string) => {
        setInputValue(inputValue.map((r) => r._id === id ? { ...r, value: newValue } : r));
    };

    const removeParamRow = (id: string) => {
        setInputValue(inputValue.filter((r) => r._id !== id));
    };
    ...
    return (
        <div className="flex min-h-screen p-8">
            ...
            <Dialog header={t("config_panel.parameters")} visible={paramsDialogOpen} style={{ width: "40rem" }} onHide={() => { setParamsDialogOpen(false); setParamsOverlay(null); setInputValue([] as ParamRow[]); lastSavedParamsRef.current = ""; }}
                footer={
                    <div className="flex justify-between w-full">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button label={t("config_panel.close")} icon="pi pi-times" severity="secondary" onClick={() => { setParamsDialogOpen(false); setParamsOverlay(null); setInputValue([] as ParamRow[]); lastSavedParamsRef.current = ""; }} />
                        </div>
                    </div>
                }
            >
